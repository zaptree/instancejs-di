...
Loader.prototype.get = function (key, injector, standardRequire, cacheLoadedModule) {
	...
	if (file) {
		...
		var customRequire = function (path) {
			if (path.match(/\//)) {
				return require(Path.resolve(file, '..', path));
			} else {
				return require(path);
			}
		};
		...
		if (cacheLoadedModule) {
			if (!loadedModules[file]) {
				mockModule = {};
				modules[file](mockModule, injector, injector.include.bind(injector), customRequire, file, Path.dirname(file));
