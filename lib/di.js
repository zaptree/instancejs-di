...
function Di(options) {
	...
	this.$options = _.merge({
		$root: this,
		$parent: null,
		objectClasses: false,	// if it will consider objects with an initialize method as `classes` and inject dependencies in the initialize method
		paths: {},
		typeMatcher: {},
		...
		if (type.factory) {
			if (_.isString(type.factory)) {
				if (_this.$options.factories[type.factory]) {
					...
					// the last parameter will allow for the loaded to cache the module
					var factory = _this.$loader.get(factoryKey, _this, null, true);
					if (factory === false) {
						throw new Error('Factory ' + type.factory + ' is not defined');
					}
					$key = _this.$factoryPrefix + key;
					_this.set('typeFactory', $key, factory);
					type.factoryKey = $key;
				}
			} else {
...
_.extend(Di.prototype, {
	...
	stub: function (key, method, value) {
		// so that I can sync return an actual spy I create the spy here and pass it as the value in the sandbox.stub
		// since getting the value returned from sandbox.stub is not possible at this point since it is created when you
		// get the stubbed module for the first time...
		// fixme: I don't want to store the stubs in the root, I think that was a bad idea. just store them at current injector, this allows for more flexibility
		key = this._convertKey(key);
		...
		if (arguments.length === 2) {
			...
			this.$stubs[key] = this.$stubs[key] || {};
			this.$stubs[key][method] = value;

		}

		return value;
	},
	spy: function (key, method) {
		var spy = sinon.spy();
		key = this._convertKey(key);
		// stub the whole module
		if (arguments.length === 1) {
			...
			this.$spies[key] = this.$spies[key] || {};
			this.$spies[key][method] = spy;
	...
	set: function (type, key, val, isValue) {
		 ...
		 */
		if (
			arguments.length !== 4 && !(arguments.length === 3 && _.isString(arguments[1]) && this.$options.types[arguments[0]]) &&
		...
		if (!isValue) {
			...
			} else if (_.isArray(val) && (_.isFunction(_.last(val)) || (this.$options.objectClasses && _.isObject(_.last(val)) && _.isFunction(_.last(val).initialize)))) {
			...
			} else if (this.$options.objectClasses && _.isObject(val) && _.isFunction(val.initialize)) {
				type = type || val.$type || this._getType(key, isValue);
				module = {
					type: type,
	...
	get: function (key) {
		...
		return mod
			...
			.then(function (value) {
				if (spies) {
					if (spies.$$stubWholeModule) {
						// if value is not method here this should of not been spied
						return this._wrapSpy(value, spies.spy);
					}

					_.each(spies, function (spy, method) {
						if (value[method].name !== 'proxy') {
							_this.$root.$sandbox.stub(value, method, _this._wrapSpy(value[method], spy));
						}
					});
	...
	_getSpies: function (key) {
		var $injector = this;
		var spies;
		do {
			spies = $injector.$spies[key];
			$injector = $injector.$parent;
		} while ($injector && !spies);
		return spies;
	},
	_getStubs: function (key) {
		var $injector = this;
		var stubs;
		do {
			stubs = $injector.$stubs[key];
			$injector = $injector.$parent;
		} while ($injector && !stubs);
		return stubs;
	},
	_wrapSpy: function (method, spy) {
		return function () {
	...
	_create: function (key, mod, resolvedDependencies) {
		...
		return Promise.resolve()
			...
			.then(function (factory) {
				...
				return factory(mod, resolvedDependencies, this, key);
			})
