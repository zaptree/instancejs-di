...
_.extend(Di.prototype, {
	include: function(key){
		...
		return this.$loader.getSync(key, this);
	...
	stub: function (module, method, value) {
		...
		if (arguments.length === 2) {
			...
			this.$root.$stubs[module] = this.$root.$stubs[module] || {};
			this.$root.$stubs[module][method] = value;
	...
	set: function (type, key, val, isValue) {
		...
		if (!isValue) {
			if (_.isFunction(val)) {
				type = type || val.$type || this._getType(key);
				...
				type = type || _.last(val).$type || this._getType(key);
	...
	_loadModule: function (key) {

		// I might want to rethink this when on the same injector we can have race conditions meaning this.set gets
		// called more than once for the same module. Could this cause any issues other than a small performance loss?
		// Also this is tricky because I don't know which scope the module actually gets set at so I can't easily
		// check. (well I would recursively check like .get does)

		return this.$loader.get(key, this)
			.bind(this)
